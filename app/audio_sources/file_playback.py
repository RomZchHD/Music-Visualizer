"""File-backed playback source and real-time analysis."""
...
from app.utils import clamp, display_title, format_seconds


class FilePlaybackSource(BaseAudioSource):
    ...
    def get_snapshot(self) -> PlaybackSnapshot:
        ...
        with self._lock:
            ...
            primary_label = "Pause" if self._state == TransportState.PLAYING else "Play"
            return PlaybackSnapshot(
                ...
                source_name=metadata.title if metadata is not None else "No file loaded",
                detail_text=f"{format_seconds(position)} / {format_seconds(duration)}",
                status_text=self._state.value,
                primary_action_label=primary_label,
                primary_action_enabled=True,
                open_file_enabled=True,
                stop_enabled=metadata is not None,
                volume_enabled=True,
